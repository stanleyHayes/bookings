export const CREATE_BOOKING_REQUEST = 'CREATE_BOOKING_REQUEST';
export const CREATE_BOOKING_SUCCESS = 'CREATE_BOOKING_SUCCESS';
export const CREATE_BOOKING_FAILURE = 'CREATE_BOOKING_FAILURE';

export const GET_BOOKING_REQUEST = 'GET_BOOKING_REQUEST';
export const GET_BOOKING_SUCCESS = 'GET_BOOKING_SUCCESS';
export const GET_BOOKING_FAILURE = 'GET_BOOKING_FAILURE';

export const GET_NEXT_BOOKING_REQUEST = 'GET_NEXT_BOOKING_REQUEST';
export const GET_NEXT_BOOKING_SUCCESS = 'GET_NEXT_BOOKING_SUCCESS';
export const GET_NEXT_BOOKING_FAILURE = 'GET_NEXT_BOOKING_FAILURE';

export const GET_CURRENT_BOOKING_REQUEST = 'GET_CURRENT_BOOKING_REQUEST';
export const GET_CURRENT_BOOKING_SUCCESS = 'GET_CURRENT_BOOKING_SUCCESS';
export const GET_CURRENT_BOOKING_FAILURE = 'GET_CURRENT_BOOKING_FAILURE';

export const GET_BOOKINGS_REQUEST = 'GET_BOOKINGS_REQUEST';
export const GET_BOOKINGS_SUCCESS = 'GET_BOOKINGS_SUCCESS';
export const GET_BOOKINGS_FAILURE = 'GET_BOOKINGS_FAILURE';

export const GET_TODAY_BOOKINGS_REQUEST = 'GET_TODAY_BOOKINGS_REQUEST';
export const GET_TODAY_BOOKINGS_SUCCESS = 'GET_TODAY_BOOKINGS_SUCCESS';
export const GET_TODAY_BOOKINGS_FAILURE = 'GET_TODAY_BOOKINGS_FAILURE';

export const UPDATE_BOOKING_REQUEST = 'UPDATE_BOOKING_REQUEST';
export const UPDATE_BOOKING_SUCCESS = 'UPDATE_BOOKING_SUCCESS';
export const UPDATE_BOOKING_FAILURE = 'UPDATE_BOOKING_FAILURE';

export const DELETE_BOOKING_REQUEST = 'DELETE_BOOKING_REQUEST';
export const DELETE_BOOKING_SUCCESS = 'DELETE_BOOKING_SUCCESS';
export const DELETE_BOOKING_FAILURE = 'DELETE_BOOKING_FAILURE';